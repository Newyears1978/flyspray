...
<?php if ($task_details['is_closed']): //if task is closed ?>
  ...
  echo tpl_form(Filters::noXSS(CreateURL('details', $task_details['task_id']))); ?>
    ...
    <button class="ggl button green"><?php echo L('reopenthistask'); ?></button>
  </form>
...
<?php else:  //if task is open  ?>
  <?php if ($user->can_close_task($task_details) && !$d_open): ?>
    <a href="<?php echo Filters::noXSS(CreateURL('details', $task_details['task_id'], null, array('showclose' => !Req::val('showclose')))); ?>"
       id="closetask" class="ggl button blue" style="float: right" accesskey="y"
    ...
    <div id="closeform" class="<?php if (Req::val('action') != 'details.close' && !Req::val('showclose')): ?>hide <?php endif; ?>popup">
        ...
        <input type="hidden" name="task_id" value="<?php echo Filters::noXSS($task_details['task_id']); ?>"/>
        <select style="width:50%" name="resolution_reason"
        ...
        <button class="ggl button green" type="submit"><?php echo Filters::noXSS(L('closetask')); ?></button>
        <br/>
  ...
  <?php elseif (!$d_open && !$user->isAnon() && !Flyspray::AdminRequestCheck(1, $task_details['task_id'])): ?>
    <a href="#close" id="reqclose" class="ggl button blue" style="float: right" onclick="showhidestuff('closeform');"><?php echo Filters::noXSS(L('requestclose')); ?></a>
  ...
  <?php elseif(!$user->isAnon()): ?>
    <a  id="reqclose" class="ggl button blue tooltip button disabled main" style="float: right"><?php echo Filters::noXSS(L('closetask')); ?>
  ...
  <?php if ($user->can_edit_task($task_details)): ?>
    <a id="edittask" class="ggl button gray" accesskey="e"
       href="<?php echo Filters::noXSS(CreateUrl('edittask', $task_details['task_id'])); ?>"><i class="fa fa-edit"></i> <?php echo Filters::noXSS(L('edittask')); ?></a>
  <?php endif; ?>
  ...
  <?php if ($user->can_take_ownership($task_details)): ?>
    <?php echo tpl_form(Filters::noXSS(CreateURL('details', $task_details['task_id'])),null,null,null,'style="display:inline"'); ?>
      ...
      <button class="ggl button orange" type="submit" id="own"><?php echo Filters::noXSS(L('assigntome')); ?></button>
    </form>
  <?php endif; ?>
  ...
  <?php if ($user->can_add_to_assignees($task_details) && !empty($task_details['assigned_to'])): ?>
    <?php echo tpl_form(Filters::noXSS(CreateURL('details', $task_details['task_id'])),null,null,null,'style="display:inline"'); ?>
      ...
      <input type="hidden" name="task_id" value="<?php echo Filters::noXSS($task_details['task_id']); ?>" />
      <input type="hidden" name="ids" value="<?php echo Filters::noXSS($task_details['task_id']); ?>" />
      <button class="ggl button orange" type="submit" id="own_add"><?php echo Filters::noXSS(L('addmetoassignees')); ?></button>
    </form>
  <?php endif; ?>
	<input type="checkbox" id="s_quickactions" />

	<label class="ggl button gray" style="float: right" id="actions" for="s_quickactions"><?php echo Filters::noXSS(L('quickaction')); ?></label>
	<div id="actionsform">
        <ul>
            <?php if ($user->can_edit_task($task_details)): ?>
            <li>
            ...
            <li><input type="checkbox" id="s_adddependent"/><label for="s_adddependent"><?php echo Filters::noXSS(L('adddependenttask')); ?></label>
              ...
              <?php echo Filters::noXSS(L('task')) . ' #'; ?> <input class="text" type="text" value="<?php echo Filters::noXSS(Req::val('dep_task_id')); ?>" id="dep_task_id" name="dep_task_id" size="5" maxlength="10" />
	...
	xmlHttp.onreadystatechange = function(){
		if(xmlHttp.readyState == 4){
			...
			if(xmlHttp.status == 200){
				if(target.getElementsByTagName("span").length > 0 && target.getElementsByTagName("span")[0].className != "np")//for progress
				{
...
<?php if($user->can_edit_task($task_details)) : ?><div id="intromessage" align="center"><?php echo Filters::noXSS(L('clicktoedit')); ?></div><?php endif; ?>
    <ul class="fieldslist">
		...
		<?php if ($user->can_edit_task($task_details)): ?>
			<span style="display:none">
				<div style="float:right">
					<select class="s2" style="width:100%" id="status" name="item_status">
				 		<?php echo tpl_options($proj->listTaskStatuses(), Req::val('item_status', $task_details['item_status'])); ?>
					</select>
					<br/><a onclick="quick_edit(this.parentNode.parentNode, 'status')" href="javascript:void(0)" class="ggl button gray"><?php echo Filters::noXSS(L('confirmedit')); ?></a><a onclick="show_hide(this.parentNode.parentNode, false)" href="javascript:void(0)" class="ggl button gray"><?php echo Filters::noXSS(L('canceledit')); ?></a>
				</div>
			</span>
		<?php endif; ?>
        ...
        <li>
            <span class="label"><?php echo Filters::noXSS(L('percentcomplete')); ?></span>
				<span <?php if ($user->can_edit_task($task_details)): ?>onclick="show_hide(this, true)"<?php endif;?> class="value">
					<div class="progress_bar_container" style="width: 100%">
	...
	<?php if ($user->can_edit_task($task_details)): ?>
		<span style="display:none">
			<div style="float:right">
				<select class="s2" style="width:100%" id="percent" name="percent_complete">
					<?php $arr = array(); for ($i = 0; $i<=100; $i+=10) $arr[$i] = $i.'%'; ?>
					<?php echo tpl_options($arr, Req::val('percent_complete', $task_details['percent_complete'])); ?>
				</select>
				<br/><a onclick="quick_edit(this.parentNode.parentNode, 'percent')" href="javascript:void(0)" class="ggl button gray"><?php echo Filters::noXSS(L('confirmedit')); ?></a><a href="javascript:void(0)" onclick="show_hide(this.parentNode.parentNode, false)" class="ggl button gray"><?php echo Filters::noXSS(L('canceledit')); ?></a>
			</div>
		</span>
	<?php endif; ?>
        ...
        <li>
            ...
            <span <?php if ($user->can_edit_task($task_details)): ?>onclick="show_hide(this, true)"<?php endif;?> class="value"><?php echo Filters::noXSS($task_details['tasktype_name']); ?></span>
 			<?php if ($user->can_edit_task($task_details)):?>
 				<span style="display:none;">
					<div style="float:right">
						<select class="s2" style="width:100%" id="tasktype" name="task_type">
							<?php echo tpl_options($proj->listTaskTypes(), Req::val('task_type', $task_details['task_type'])); ?>
						</select>
						<br/><a onclick="quick_edit(this.parentNode.parentNode, 'tasktype')" href="javascript:void(0)" class="ggl button gray"><?php echo Filters::noXSS(L('confirmedit')); ?></a><a href="javascript:void(0)" onclick="show_hide(this.parentNode.parentNode, false)" class="ggl button gray"><?php echo Filters::noXSS(L('canceledit')); ?></a></span>
					</div>
				</span>
			<?php endif; ?></li>
        <?php endif; ?>
	...
	<?php if ($user->can_edit_task($task_details)): ?>
		<span style="display:none">
			<div style="float:right">
				<select class="s2" style="width:100%" id="category" name="product_category">
			 		<?php echo tpl_options($proj->listCategories(), Req::val('product_category', $task_details['product_category'])); ?>
				</select>
				<br/><a onclick="quick_edit(this.parentNode.parentNode, 'category')" href="javascript:void(0)" class="ggl button gray"><?php echo Filters::noXSS(L('confirmedit')); ?></a><a href="javascript:void(0)" onclick="show_hide(this.parentNode.parentNode, false)" class="ggl button gray"><?php echo Filters::noXSS(L('canceledit')); ?></a>
			</div>
		</span>
        <?php endif; ?>
        </li>
	...
	<?php if ($user->can_edit_task($task_details)): ?>
		<span style="display:none">
			<div style="float:right">
				<select class="s2" style="width:100%" id="os" name="operating_system">
					<?php echo tpl_options($proj->listOs(), Req::val('operating_system', $task_details['operating_system'])); ?>
				</select>
				<br/><a onclick="quick_edit(this.parentNode.parentNode, 'os')" href="javascript:void(0)" class="ggl button gray"><?php echo Filters::noXSS(L('confirmedit')); ?></a><a href="javascript:void(0)" onclick="show_hide(this.parentNode.parentNode, false)" class="ggl button gray"><?php echo Filters::noXSS(L('canceledit')); ?></a>
			</div>
		</span>
        <?php endif; ?>
        </li>
	...
	<?php if ($user->can_edit_task($task_details)): ?>
		<span style="display:none">
			<div style="float:right">
				<select class="s2" style="width:100%" id="severity" name="task_severity">
			 		<?php echo tpl_options($fs->severities, Req::val('task_severity', $task_details['task_severity'])); ?>
				</select>
				<br/><a onclick="quick_edit(this.parentNode.parentNode, 'severity')" href="javascript:void(0)" class="ggl button gray"><?php echo Filters::noXSS(L('confirmedit')); ?></a><a href="javascript:void(0)" onclick="show_hide(this.parentNode.parentNode, false)" class="ggl button gray"><?php echo Filters::noXSS(L('canceledit')); ?></a></div>
		</span>
        <?php endif; ?>
        </li>
        <?php endif; ?>
        ...
        <li>
            ...
            <span <?php if ($user->can_edit_task($task_details)): ?>onclick="show_hide(this, true)"<?php endif;?> class="value pri<?php echo $task_details['task_priority'];?>"><span class="np"> <?php echo Filters::noXSS($task_details['priority_name']); ?></span></span>

	<?php if ($user->can_edit_task($task_details)): ?>
		<span style="display:none">
			<div style="float:right">
				<select style="width:100%" id="priority" name="task_priority">
			 		<?php echo tpl_options($fs->priorities, Req::val('task_priority', $task_details['task_priority'])); ?>
				</select>
				<br/><a onclick="quick_edit(this.parentNode.parentNode, 'priority')" href="javascript:void(0)" class="ggl button gray"><?php echo Filters::noXSS(L('confirmedit')); ?></a><a href="javascript:void(0)" onclick="show_hide(this.parentNode.parentNode, false)" class="ggl button gray"><?php echo Filters::noXSS(L('canceledit')); ?></a>
			</div>
		</span>
        <?php endif; ?>
        </li>
	...
	<?php if ($user->can_edit_task($task_details)): ?>
		<span style="display:none">
			<div style="float:right">
				<select class="s2" style="width:100%" id="reportedver" name="product_version">
				...
				<br/><a onclick="quick_edit(this.parentNode.parentNode, 'reportedver')" href="javascript:void(0)" class="ggl button gray"><?php echo Filters::noXSS(L('confirmedit')); ?></a><a href="javascript:void(0)" onclick="show_hide(this.parentNode.parentNode, false)" class="ggl button gray"><?php echo Filters::noXSS(L('canceledit')); ?></a>
			</div>
		</span>
        <?php endif; ?>
        </li>
	...
	<?php if ($user->can_edit_task($task_details)): ?>
		<span style="display:none">
			<div style="float:right">
				<select class="s2" style="width:100%" id="dueversion" name="closedby_version">
				...
				<br/><a onclick="quick_edit(this.parentNode.parentNode, 'dueversion')" href="javascript:void(0)" class="ggl button gray"><?php echo Filters::noXSS(L('confirmedit')); ?></a><a href="javascript:void(0)" onclick="show_hide(this.parentNode.parentNode, false)" class="ggl button gray"><?php echo Filters::noXSS(L('canceledit')); ?></a>
			</div>
		</span>
        <?php endif; ?>
	    ...
	    <span <?php if ($user->can_edit_task($task_details)): ?>onclick="show_hide(this, true)"<?php endif;?> class="value">
			<?php echo Filters::noXSS(formatDate($task_details['due_date'], false, L('undecided'))); ?><br><?php
				...
				if($task_details['due_date'] > 0)
				{
					...
					{
						echo "<span style='color: red; font-weight: bold'>".$days." ".L('daysleft')."</span>";
					}
					elseif($days < 0)
					{
						echo "<span style='color: red; font-weight: bold'>".str_replace('-', '', $days)."
                        ".L('dayoverdue')."</span>";
					}
					elseif($days == 0)
					{
						echo "<span style='color: red; font-weight: bold'>".L('duetoday')."</span>";
					}
					else
					{
	...
	<?php if ($user->can_edit_task($task_details)): ?>
		<span style="display:none">
			<div style="float:right">
				...
				<br/><a onclick="quick_edit(this.parentNode.parentNode, 'due_date')" href="javascript:void(0)" class="ggl button gray"><?php echo Filters::noXSS(L('confirmedit')); ?></a><a href="javascript:void(0)" onclick="show_hide(this.parentNode.parentNode, false)" class="ggl button gray"><?php echo Filters::noXSS(L('canceledit')); ?></a>
			</div>
		</span>
        <?php endif; ?>
        ...
        <li>
        	...
        	<span style="display:none">
        		<div style="float:right">
    	    		...
    	    		<br/><a onclick="quick_edit(this.parentNode.parentNode, 'estimatedeffort')" href="javascript:void(0)" class="ggl button gray"><?php echo Filters::noXSS(L('confirmedit')); ?></a> <a href="javascript:void(0)" onclick="show_hide(this.parentNode.parentNode, false)" class="ggl button gray"><?php echo Filters::noXSS(L('canceledit')); ?></a>
    	    	</div>
        	</span>
        	<?php endif; ?>
        </li>
        ...
        <li class="votes">
			...
			<span class="value">
				<?php if (count($votes)): ?>
					...
					<div id="showvotes" class="hide">
						...
						<?php foreach ($votes as $vote): ?>
							<li><?php echo tpl_userlink($vote); ?> <small>(<?php echo Filters::noXSS(formatDate($vote['date_time'])); ?>)</small></li>
						<?php endforeach; ?>
						</ul>
					</div>
				<?php endif; ?>
				...
				<?php elseif ($user->can_vote($task_details) == -2): ?>	<small>(<?php echo Filters::noXSS(L('alreadyvotedthistask')); ?>)</small>
				<?php elseif ($user->can_vote($task_details) == -3): ?> <small>(<?php echo Filters::noXSS(L('alreadyvotedthisday')); ?>)</small>
				<?php elseif ($user->can_vote($task_details) == -4): ?> <small>(<?php echo Filters::noXSS(L('votelimitreached')); ?>)</small>
				<?php endif; ?>
			</span>
		</li>
		<?php endif; ?>
...
<div id="taskdetailsfull" class="task-details">
	<span class="task-id"><?php echo 'FS#' . Filters::noXSS($task_details['task_id']); ?></span>
	<span class="caption caption-<?php echo preg_replace('/[^a-zA-Z]+/', '', strtolower($task_details['status_name'])); ?>"><?php echo $task_details['status_name'];?></span><br />
	<h2 class="summary severity<?php echo Filters::noXSS($task_details['task_severity']); ?>">
		<?php echo Filters::noXSS($task_details['item_summary']); ?>
	</h2>
	...
	<div class="task-author">
		<?php echo Filters::noXSS(L('openedby')) . ' ' . tpl_userlink($task_details['opened_by']) . ' - ' . Filters::noXSS(formatDate($task_details['date_opened'], false)); ?>
		<?php echo tpl_userlinkavatar($task_details['opened_by'], $fs->prefs['max_avatar_size'], 'task-avatar'); ?>
	</div>
	<div class="task-details-text"><?php echo $task_text; ?></div>
...
<div id="taskinfo">
        ...
        ?>
        <table id="dependency_table" class="table table-nonfluid" width="100%">
            ...
            <tr>
                ...
                <td>
                    ...
                    <button type="submit" title="<?php echo Filters::noXSS(L('remove')); ?>" class="ggl button red"><i class="fa fa-unlink fa-lg"></i></button>
                    </form>
                </td>
            </tr>
            <?php endforeach; ?>
        ...
        #$projects = $fs->listProjects();
        ?>
        <table id="blocking_table" class="table table-nonfluid" width="100%">
