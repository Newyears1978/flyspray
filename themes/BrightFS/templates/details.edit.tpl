...
<div id="taskdetails">
	...
	<li<?php echo in_array('status', $fields) ? '' : ' style="display:none"'; ?>>
		...
		<select class="s2" id="status" name="item_status" <?php echo tpl_disableif(!$user->perms('modify_all_tasks')); ?>>
	...
	<li<?php echo in_array('progress', $fields) ? '' : ' style="display:none"'; ?>>
		...
		<select class="s2" id="percent" name="percent_complete" <?php echo tpl_disableif(!$user->perms('modify_all_tasks')) ?>>
	...
	<li<?php echo in_array('tasktype', $fields) ? '' : ' style="display:none"'; ?>>
		...
		<select class="s2" id="tasktype" name="task_type">
	...
	<li<?php echo in_array('category', $fields) ? '' : ' style="display:none"'; ?>>
		...
		<?php
		$catselect['attr']['class'] = 's2';
		echo tpl_select($catselect); ?>
	</li>
	<!-- Assigned To-->
	...
	<li<?php echo in_array('os', $fields) ? '' : ' style="display:none"'; ?>>
		...
		<select class="s2" id="os" name="operating_system">
	...
	<li<?php echo in_array('severity', $fields) ? '' : ' style="display:none"'; ?>>
		...
		<select class="s2" id="severity" name="task_severity">
	...
	<li<?php
		...
		<select class="s2" id="reportedver" name="reportedver">
			<?php
	...
	<li<?php echo in_array('dueversion', $fields) ? '' : ' style="display:none"'; ?>>
		...
		<select class="s2" id="dueversion" name="closedby_version" <?php echo tpl_disableif(!$user->perms('modify_all_tasks')) ?>>
	...
	<li<?php echo (count($fs->projects) > 1) ? '' : ' style="display:none"'; ?>>
		...
		<select class="s2" name="project_id" id="project_id">
...
<div id="taskdetailsfull">
	<label for="itemsummary"<?php echo isset($_SESSION['ERRORS']['summaryrequired']) ? ' class="summary errorinput" title="'.eL('summaryrequired').'"':' class="summary"'; ?>><?php echo 'FS#' . Filters::noXSS($task_details['task_id']); ?> <?php echo Filters::noXSS(L('summary')); ?>:
	...
	echo $brightFS->dokuwiki('detailed_desc', 'details', 15, Req::val('detailed_desc', $task_details['detailed_desc']), (defined('FLYSPRAY_HAS_PREVIEW') ? 'preview' : null), $baseurl);
	?>
	
	<br />
	<button class="ggl button gray" type="reset"><?php echo Filters::noXSS(L('reset')); ?></button>
	<br />
	...
	<div class="row">
		...
		<?php if ($user->perms('create_attachments')): ?>
			...
			<button id="addlinkbox_addalink" tabindex="10" class="ggl button gray" style="width:100%;" type="button" onclick="addLinkField('addlinkbox')"><?php echo Filters::noXSS(L('addalink')); ?></button>
			<button id="addlinkbox_addanotherlink" tabindex="10" class="ggl button gray" style="width:100%; display: none;" type="button" onclick="addLinkField('addlinkbox')"><?php echo Filters::noXSS(L('addalink')); ?></button>
			<br />
			<span style="display: none"><?php /* this span is shown/copied by javascript when adding links */ ?>
				...
				<a href="javascript://" class="ggl button red" tabindex="9" title="<?php echo Filters::noXSS(L('remove')); ?>" onclick="removeLinkField(this, 'addlinkbox');"><i class="fa fa-remove fa-lg"></i></a><br />
			</span>
		<?php endif; ?>
		</div>
		...
		if ($user->perms('create_attachments')): ?>
			...
			<button id="uploadfilebox_attachafile" tabindex="7" class="ggl button gray" style="width:100%;" type="button" onclick="addUploadFields()">
			...
			<button id="uploadfilebox_attachanotherfile" tabindex="7" class="ggl button gray" style="width:100%; display: none;" type="button" onclick="addUploadFields()">
			...
			<span style="display: none"><?php /* this span is shown/copied by javascript when adding files */ ?>
				...
				<a href="javascript://" tabindex="6" class="ggl button red" title="<?php echo Filters::noXSS(L('remove')); ?>" onclick="removeUploadField(this);"><i class="fa fa-remove fa-lg"></i></a><br />
			</span>
		<?php endif; ?>
		</div>
	</div>
	...
	<div class="row">
		...
		<textarea accesskey="r" tabindex="8" id="comment_text" name="comment_text" rows="2"></textarea>
	</div>
	<?php endif; ?>
	<div class="row">
		<button type="submit" class="ggl button green" accesskey="s" onclick="return checkok('<?php echo Filters::noJsXSS($baseurl); ?>js/callbacks/checksave.php?time=<?php echo Filters::noXSS(time()); ?>&amp;task_id=<?php echo Filters::noXSS($task_details['task_id']); ?>', '<?php echo Filters::noJsXSS(L('alreadyedited')); ?>', 'taskeditform')"><?php echo Filters::noXSS(L('savedetails')); ?></button>
		<a class="ggl button gray" href="<?php echo Filters::noXSS(CreateUrl('details', $task_details['task_id'])); ?>"><?php echo Filters::noXSS(L('canceledit')); ?></a>
	</div>
</div>
<div class="clear"></div>
</div>
