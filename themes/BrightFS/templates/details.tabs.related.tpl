<div id="related" class="tab">
      ...
      <table id="tasks_related" class="userlist">
        ...
        <tr>
          <td colspan="2">
            ...
            <button class="ggl button gray" type="submit"><?php echo Filters::noXSS(L('remove')); ?></button>
          </td>
        </tr>
        </tfoot>
      </table>
  ...
  <?php echo tpl_form(Filters::noXSS(CreateUrl('details', $task_details['task_id'])).'#related',null,null,null,'class="clear" id="formaddrelatedtask"'); ?>
    <div>
      ...
      <input type="hidden" name="task_id" value="<?php echo Filters::noXSS($task_details['task_id']); ?>" />
      <label><?php echo Filters::noXSS(L('addnewrelated')); ?> <?php echo Filters::noXSS(L('task')) . ' #'; ?><input name="related_task" id="related_task_input" type="text" class="text" size="10" maxlength="10" /></label>
      <button class="ggl button green" type="submit" onclick="return checkok('<?php echo Filters::noJsXSS($baseurl); ?>js/callbacks/checkrelated.php?related_task=' + $('related_task_input').value + '&amp;project=<?php echo Filters::noXSS($proj->id); ?>', '<?php echo Filters::noJsXSS(L('relatedproject')); ?>', 'formaddrelatedtask')"><?php echo Filters::noXSS(L('add')); ?></button>
    </div>
  </form>
  <?php endif; ?>
</div>
